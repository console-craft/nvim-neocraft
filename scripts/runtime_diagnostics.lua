-- Collect live diagnostics from the lua_ls LSP client for all Lua files in the Neocraft configuration.
...
local function relative_path(path) return vim.fs.relpath(config_root, path) or path end
...
local function print_report(reports)
  ...
  if total_diagnostics == 0 and unattached_files == 0 then
    ...
    print(string.format('Total live Lua diagnostics: %d in %d file(s)\n', total_diagnostics, files_with_diagnostics))
  end
...
local function wait_for_client(bufnr, timeout)
  return vim.wait(timeout, function() return #vim.lsp.get_clients({ bufnr = bufnr, name = client_name }) > 0 end, 50)
end

local function inspect_path(path, index, total)
  print('[' .. index .. '/' .. total .. '] Inspecting ' .. relative_path(path) .. '...\n')
...
local function default_paths()
  ...
  for _, dir_name in ipairs({ 'lua', 'after', 'colors' }) do
...
local group = vim.api.nvim_create_augroup('neocraft_runtime_diagnostics', { clear = true })

vim.api.nvim_create_autocmd('DiagnosticChanged', {
  group = group,
  callback = function(args) published[args.buf] = true end,
})

local reports = {}

for index, path in ipairs(default_paths()) do
  table.insert(reports, inspect_path(path, index, #default_paths()))
end
